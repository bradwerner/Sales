...
Updates: v1 - 1/29/2018
	...
	v3 - 1/31/2019 - The initial query is for testing to get the data.  The second 2 queries are for inserting the data.
***************************/
Select
	ad.[date],
...
/***********
Insert the data from the above query straight into the budget table
***********/

insert into it.dbo.BUDGET
select
...
FROM
	[dev-sql1].BI360DW.dbo.f_Trans_GL gl
	inner join
	[dev-sql1].BI360DW.dbo.d_account acct
		on gl.account = acct.MemberId
	Inner join
	[dev-sql1].BI360DW.dbo.d_dim2 di
		on gl.Dim2 = di.MemberID
	Inner Join
	[bludot-sql1].it.dbo.acct_date ad 
		on left(gl.TimePeriod, 4) = ad.Year_Number
...
/*****************
taking records from 2018 that have no 2019 budget and putting them into the system for 2019 with a 0
*********/
...
Select
	...
	ad.Year_Number,
	ad.Month_Number,
	c.[Customer Class - Original],
	0 as [Budget]
from
	it.dbo.acct_date ad
	Inner join
(
Select b.[Customer Class - Original]
from

(Select distinct [Customer Class - Original] from it.dbo.BUDGET where Budget_Year = 2019) a
right join 
(Select distinct [Customer Class - Original] from it.dbo.BUDGET where Budget_Year = 2018) b
	on a.[Customer Class - Original] = b.[Customer Class - Original]
Where
	a.[Customer Class - Original] is null
) c
	on ad.Year_Number = 2019
